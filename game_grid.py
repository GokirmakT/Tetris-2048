...
class GameGrid:
    ...
    def __init__(self, grid_h, grid_w):
        ...
        self.line_color = Color(114, 114, 114)
        self.boundary_color = Color(255, 255, 255)
    ...
    def display(self, High_Score):
        ...
        stddraw.text(14, 14, "HIGH SCORE:")
        stddraw.rectangle(12, 11, 4, 2)
        stddraw.boldText(14, 12, str(High_Score))
    ...
    def check_row(self):
        ...
        for row in range(n_rows):
            ...
            if counter == n_cols:
                will_row_clears.append(row)

        return will_row_clears

    def clear_row(self):
        clear_list = self.check_row()
            ...
            for col in range(n_cols):
                if self.tile_matrix[row][col] != None and self.tile_matrix[row + 1][col] != None:
                    if self.tile_matrix[row][col].number == self.tile_matrix[row + 1][col].number:
                       self.tile_matrix[row][col].number = self.tile_matrix[row][col].number * 2
                       newScore = self.tile_matrix[row][col].number
                       self.total_score += newScore
                       self.tile_matrix[row + 1][col] = None
                       self.gravity_merge(row, col)

    def gravity_merge(self, row, col):

...
class Tetromino:
   ...
   def move(self, direction, game_grid):
      ...
      if direction == "left":
         ...
         self.tile_matrix = [[self.tile_matrix[j][i] for j in range(len(self.tile_matrix))] for i in range(len(self.tile_matrix[0]))]
         for i in range(len(self.tile_matrix)):

            start = 0
            end = len(self.tile_matrix) - 1

            while (start < end):
               # Swap the element
               self.tile_matrix[i][start], self.tile_matrix[i][end] = self.tile_matrix[i][end], self.tile_matrix[i][start];
      ...
      elif direction == "Counter-Clock-wise":
         self.tile_matrix = [[self.tile_matrix[j][i] for j in range(len(self.tile_matrix))] for i in range(len(self.tile_matrix[0]) -1, -1, -1)]
